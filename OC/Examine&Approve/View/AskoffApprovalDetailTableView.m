//
//  AskoffApprovalDetailTableView.m
//  JingTeYuHui
//
...
#import "PCHeader.h"
@interface AskoffApprovalDetailTableView () <UITableViewDelegate, UITableViewDataSource>
...
@implementation AskoffApprovalDetailTableView
...
- (void)setViewModel:(ApplyDetailViewModel *)viewModel {
    ...
    self.titleArr = [NSMutableArray arrayWithArray:@[@[@"审批编号：",@"所在部门：",@"请假类型：",@"申请事由：",@"开始时间：",@"结束时间：",@"请假时长："],@[@"审批流程"]]];
    self.contentArr = [NSMutableArray arrayWithArray:@[@[NSStringFormate(@"%@",viewModel.ApproveInfo[@"eventNo"]),@"所在部门",arr.count?arr[0][@"Type"]:@"",arr.count?arr[0][@"Remark"]:@"",arr.count?arr[0][@"StartTime"]:@"",arr.count?arr[0][@"EndTime"]:@"",NSStringFormate(@"%@时",arr.count?arr[0][@"Hours"]:@""),@[@""]],@[@""]]];
