//
//  DiscountApplyDetailTableView.m
//  JingTeYuHui
//
...
#import "DiscountApplyDetailTableView.h"
#import "ApplyDetailTableView.h"
#import "PCHeader.h"
@interface DiscountApplyDetailTableView () <UITableViewDelegate, UITableViewDataSource>
...
@implementation DiscountApplyDetailTableView
...
- (void)setViewModel:(ApplyDetailViewModel *)viewModel {
    ...
    self.titleArr = [NSMutableArray arrayWithArray:@[@[@"审批编号：",@"所在部门：",@"打折房间：",@"打折账单：",@"申请事由：",@"打折额度：",@"发生日期：",@"备注："],@[@"审批流程"]]];
    self.contentArr = [NSMutableArray arrayWithArray:@[@[NSStringFormate(@"%@",viewModel.ApproveInfo[@"eventNo"]),@"所在部门",arr.count?arr[0][@"RoomName"]:@"",arr.count?arr[0][@"BillNo"]:@"",viewModel.ApproveInfo[@"remark"]?viewModel.ApproveInfo[@"remark"]:@"",arr.count?NSStringFormate(@"%.1f折",[arr[0][@"Discount"] floatValue]/10.):@"",BUSINESSDATE,arr.count?arr[0][@"Remark"]:@""],@[@""]]];
