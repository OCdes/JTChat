...
@interface AdvanceApplyTableView () <UITableViewDelegate, UITableViewDataSource, UICollectionViewDelegate, UICollectionViewDataSource, TZImagePickerControllerDelegate, UITextFieldDelegate, UITextViewDelegate>
...
@implementation AdvanceApplyTableView
...
- (instancetype)initWithFrame:(CGRect)frame style:(UITableViewStyle)style {
    if (self = [super initWithFrame:frame style:style]) {
        ...
        self.titleMArray = [NSMutableArray arrayWithArray:@[@[@"费用类型"],@[@"申请事由"],@[@"费用金额"],@[@"发生日期"],@[@"备注"],@[@"审批流程"],@[@"抄送人"]]];
        self.keyboardTypeMArr = [NSMutableArray arrayWithArray:@[@[@(UIKeyboardTypeDefault)],@[@(UIKeyboardTypeDefault)],@[@(UIKeyboardTypeDecimalPad)],@[@(UIKeyboardTypeDefault)],@[@(UIKeyboardTypeDefault)],@[@(UIKeyboardTypeDefault)],@[@(UIKeyboardTypeDefault)]]];
        self.requreMArray = [NSMutableArray arrayWithArray:@[@[@(1)],@[@(1)],@[@(1)],@[@(1)],@[@(0)],@[@(1)],@[@(0)]]];
        self.accessTypeArray = [NSMutableArray arrayWithArray:@[@[@(UITableViewCellAccessoryDisclosureIndicator)],@[@(UITableViewCellAccessoryNone)],@[@(UITableViewCellAccessoryNone)],@[@(UITableViewCellAccessoryDisclosureIndicator)],@[@(0)],@[@(UITableViewCellAccessoryNone)],@[@(UITableViewCellAccessoryNone)]]];
...
- (void)subBtnClicked:(UIButton *)btn {
    [self.viewModel.submitAdvanceCommand execute:nil];
    
}

- (void)setViewModel:(PurchaseApplyViewModel *)viewModel {
    _viewModel = viewModel;
    self.contentMArray = [NSMutableArray arrayWithArray:@[@[viewModel.applyType?viewModel.applyType:@""],@[viewModel.applyReason?viewModel.applyReason:@""],@[viewModel.money?viewModel.money:@""],@[viewModel.applyDate?viewModel.applyDate:@""],@[viewModel.applyNote?viewModel.applyNote:@""],@[@""],@[@""]]];
    [self reloadData];
}
...
- (void)textFieldDidEndEditing:(UITextField *)textField {
    ...
    if ([titleStr isEqualToString:@"费用金额"]) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    NSString *titleStr = self.titleMArray[indexPath.section][indexPath.row];
    if ([titleStr isEqualToString:@"费用类型"]) {
        ...
        NSArray *arr = @[@"差旅费",@"办公费",@"招待费",@"市内交通费",@"通讯费",@"用款付费",@"其他"];
