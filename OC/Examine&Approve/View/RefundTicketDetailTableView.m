...
#import "RefundTicketDetailTableView.h"
#import "ApplyDetailTableView.h"
#import "PCHeader.h"

@interface RefundTicketDetailTableView () <UITableViewDelegate, UITableViewDataSource>
...
@implementation RefundTicketDetailTableView
...
- (void)setViewModel:(ApplyDetailViewModel *)viewModel {
    ...
    self.titleArr = [NSMutableArray arrayWithArray:@[@[@"审批编号：",@"所在部门：",@"退票房间：",@"退票金额：",@"申请事由：",@"发生日期：",@"备注："],@[@"审批流程"]]];
    self.contentArr = [NSMutableArray arrayWithArray:@[@[NSStringFormate(@"%@",viewModel.ApproveInfo[@"eventNo"]),@"所在部门",arr.count?arr[0][@"RoomName"]:@"",arr.count?arr[0][@"Amount"]:@"",viewModel.ApproveInfo[@"remark"]?viewModel.ApproveInfo[@"remark"]:@"",arr.count?arr[0][@"RefundDate"]:@"",arr.count?arr[0][@"Remark"]:@""],@[@""]]];
