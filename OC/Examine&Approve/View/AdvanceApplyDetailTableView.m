...
#import "AdvanceApplyDetailTableView.h"
#import "ApplyDetailTableView.h"
#import "PCHeader.h"
@interface AdvanceApplyDetailTableView () <UITableViewDelegate, UITableViewDataSource>
...
@implementation AdvanceApplyDetailTableView
...
- (void)setViewModel:(ApplyDetailViewModel *)viewModel {
    ...
    self.titleArr = [NSMutableArray arrayWithArray:@[@[@"审批编号：",@"审批类型：",@"所在部门：",@"申请事由：",@"费用金额：",@"发生日期：",@"备注："],@[@"审批流程"]]];
    self.contentArr = [NSMutableArray arrayWithArray:@[@[NSStringFormate(@"%@",viewModel.ApproveInfo[@"eventNo"]),viewModel.ApproveInfo[@"applyType"]?viewModel.ApproveInfo[@"applyType"]:@"",@"所在部门",viewModel.ApproveInfo[@"remark"]?viewModel.ApproveInfo[@"remark"]:@"",arr.count?arr[0][@"TotalFees"]:@"",arr.count?arr[0][@"RequiredDate"]:@"",arr.count?arr[0][@"Remark"]:@""],@[@""]]];
