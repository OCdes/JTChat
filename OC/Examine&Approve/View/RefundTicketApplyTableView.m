...
@interface RefundTicketApplyTableView () <UITableViewDelegate, UITableViewDataSource, UICollectionViewDelegate, UICollectionViewDataSource, TZImagePickerControllerDelegate, UITextFieldDelegate, UITextViewDelegate>
...
@implementation RefundTicketApplyTableView
...
- (instancetype)initWithFrame:(CGRect)frame style:(UITableViewStyle)style {
    if (self = [super initWithFrame:frame style:style]) {
        ...
        self.titleMArray = [NSMutableArray arrayWithArray:@[@[@"申请事由"],@[@"退票房间"],@[@"退票金额"],@[@"发生日期"],@[@"备注"],@[@"审批流程"],@[@"抄送人"]]];
        self.keyboardTypeMArr = [NSMutableArray arrayWithArray:@[@[@(UIKeyboardTypeDefault)],@[@(UIKeyboardTypeDefault)],@[@(UIKeyboardTypeDecimalPad)],@[@(UIKeyboardTypeDefault)],@[@(UIKeyboardTypeDefault)],@[@(UIKeyboardTypeDefault)],@[@(UIKeyboardTypeDefault)],@[@(UIKeyboardTypeDefault)]]];
        self.requreMArray = [NSMutableArray arrayWithArray:@[@[@(1)],@[@(1)],@[@(1)],@[@(1)],@[@(0)],@[@(1)],@[@(0)]]];
        self.accessTypeArray = [NSMutableArray arrayWithArray:@[@[@(0)],@[@(UITableViewCellAccessoryDisclosureIndicator)],@[@(0)],@[@(0)],@[@(0)],@[@(0)],@[@(0)]]];
        self.placeHolderMArray = [NSMutableArray arrayWithArray:@[@[@"请输入"],@[@"请选择"],@[@"金额"],@[@""],@[@"请输入"],@[@""],@[@""]]];
...
- (void)subBtnClicked:(UIButton *)btn {
    [self.viewModel.submitRefundTicketCommand execute:nil];
    
}

- (void)setViewModel:(RefundAndDisconutApplyViewModel *)viewModel {
    _viewModel = viewModel;
    self.contentMArray = [NSMutableArray arrayWithArray:@[@[viewModel.applyReason?viewModel.applyReason:@""],@[viewModel.applyRoom?viewModel.applyRoom:@""],@[viewModel.money?viewModel.money:@""],@[BUSINESSDATE],@[@""],@[@""],@[@""]]];
    self.selectMarr = [NSMutableArray array];
    for (NSDictionary *dict in viewModel.selectArr) {
        [self.selectMarr addObject:dict[@"openRoomName"]];
    }
    [self reloadData];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([titleStr isEqualToString:@"申请事由"] || [titleStr isEqualToString:@"备注"]) {
        ...
        cell.tf.userInteractionEnabled = [titleStr isEqualToString:@"发生日期"] ? NO : !accessType;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    NSString *titleStr = self.titleMArray[indexPath.section][indexPath.row];
    if ([titleStr isEqualToString:@"退票房间"]) {
        if (self.viewModel.selectArr.count) {
            ...
            NSArray *arr = [NSMutableArray arrayWithArray:self.selectMarr];
            ...
            [alertView.sureSubject subscribeNext:^(id  _Nullable x) {
                ...
                self.viewModel.applyRoom = dict[@"房台名称"];
                self.viewModel.money = NSStringFormate(@"%@",dict[@"金额"]);
                self.viewModel.ticketID = NSStringFormate(@"%@",dict[@"RecID"]);
                self.viewModel.discountBill = NSStringFormate(@"%@",dict[@"帐单号"]);
                self.contentMArray[1] = @[dict[@"房台名称"]];
                self.contentMArray[2] = @[self.viewModel.money];
                [self reloadData];
            }];
        } else {
            NSString *contentStr = [self.viewModel.type isEqualToString:@"3"] ? @"当前暂无可退台票":@"当前暂无可打折房间";
